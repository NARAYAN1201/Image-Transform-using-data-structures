...
Name:Narayan Sharma
Email:narayankaushik16@gmail.com
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
    ...
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      double centre = sqrt(((centerX-x)*(centerX-x)) + ((centerY-y)*(centerY-y)));
      double p =pixel.l-(((centre*0.5)/100.0)*pixel.l);
      if((centre*0.5)<=80)
      {
          pixel.l=p;
      }
...
for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      else if(pixel.h>=102.5 && pixel.h<293.5)
      {
          pixel.h=216;
      }
    }
